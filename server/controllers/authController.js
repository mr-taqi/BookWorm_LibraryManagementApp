...
export const register = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    const registationAttemptsByUser = await User.find({
      email,
      accountVerified: false,
    });

    if (registationAttemptsByUser.length >= 5) {
      return next(
        new ErrorHandler(
...
export const verifyOTP = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new ErrorHandler("Internal server error occurred while verifying OTP.", 500)
    );
  }
});
...
export const forgotPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: "Password Recovery - (BookWorm Library Management System)",
      message
    });
...
export const resetPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  if (
    ...
    return next(
      new ErrorHandler(
        "Password must be between 8 and 16 characters long.", 400)
    );
  }
