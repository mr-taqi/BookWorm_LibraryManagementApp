...
const SideBar = ({ isSideBarOpen, setIsSideBarOpen, setSelectedComponent }) => {
  ...
  return (
    <>
      <aside className={`${isSideBarOpen ? "left-0" : "-left-full"} z-10 transition-all duration-700 md:lerative md:left-0 flex w-64 bg-black text-white flex-col h-full`}
        ...
        <nav className="flex-1 px-6 space-y-2">
          ...
          {isAuthenticated && user?.role === "Admin" && (
            <>
              ...
              <button className="w-full py-2 font-medium bg-transparent rounded-md hover:cursor-pointer flex items-center space-x-2"
              onClick={()=> dispatch(togglAddNewAdminPopup())} 
              >
