...
const UserDashboard = () => {
  ...
  useEffect(() => {
    ...
    let numberOfTotalBorrowedBooks = userBorrowedBooks?.filter((book) => (
      book.returned === false
    ));

    let numberOfTotalReturnedBooks = userBorrowedBooks?.filter((book) => (
      book.returned === true
    ));
          ...
          <div className="flex flex-col gap-7 flex-[4]">
            <div className="flex flex-col lg:flow-row gap-7 overflow-y-hidden">
              <div className="flex items-center gap-3 bg-white p-5 min-h-[120px] overflow-y-hidden rounded-lg transition hover:shadow-inner duration-300">
                ...
                <span className="bg-gray-300 h-20 lg:h-full min-w-20 flex justify-center items-center rounded-lgl">
                  <img
                    src={bookIcon} alt="Book Icon"
                    className="w-8 h-8"
                  />
                </span>
                <p className="text-lg font-semibold xl:text-xl">Your Borrowed Book List</p>
              </div>
              ...
              <div className="flex items-center gap-3 bg-white p-5 min-h-[120px] overflow-y-hidden rounded-lg transition hover:shadow-inner duration-300">
                ...
                <span className="bg-gray-300 h-20 lg:h-full min-w-20 flex justify-center items-center rounded-lg">
                  <img
                    src={returnIcon} alt="Return Icon"
                    className="w-8 h-8"
                  />
                </span>
                <p className="text-lg font-semibold xl:text-xl">Your Returned Book List</p>
              </div>
            </div>
            ...
            <div className="flex flex-col lg:flex-row gap-7">
              <div className="flex items-center gap-3 bg-white p-5 max-h-[120px] overflow-y-hidden rounded-lg transition hover:shadow-inner duration-300">
                ...
                <span className="bg-gray-300 h-20 lg:h-full min-w-20 flex justify-center items-center rounded-lg">
                  <img
                    src={browseIcon} alt="Browse  Icon"
                    className="w-8 h-8"
                  />
                </span>
                <p className="text-lg font-semibold xl:text-xl">Let's browse book inventory...</p>
              </div>
