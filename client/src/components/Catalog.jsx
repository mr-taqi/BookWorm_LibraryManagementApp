  ...
  return <>
    <main className="relative flex-1 p-6 pt-28">
      ...
      <header className="flex flex-col gap-1 sm:flex-row md:items-center mt-5">
        <button
          className={`relative rounded sm:rounded-tr-none sm:rounded-br-none sm:rounded-tl-lg sm:rounded-bl-lg text-center border-2 font-semibold py-2 w-full sm:w-72 ${filter === "borrowed" ? "bg-black text-white border-black" : "bg-gray-200 text-black border-gray-200 hover:bg-gray-300"}`}
          onClick={() => setFilter("borrowed")}
        >
          Borrowed Books
        </button>
        <button
          className={`relative rounded sm:rounded-tl-none sm:rounded-bl-none sm:rounded-tr-lg sm:rounded-br-lg text-center border-2 font-semibold py-2 w-full sm:w-72 ${filter === "overdue" ? "bg-black text-white border-black" : "bg-gray-200 text-black border-gray-200 hover:bg-gray-300"}`}
...

      {
        booksToDisplay && booksToDisplay.length > 0 ? (
          ...
          <h3 className="mt-5 text-3xl font-medium text-center">No {filter === "borrowed" ? "Borrowed" : "Overdue"} Books Found.</h3>
        ) 
      }
    </main>
